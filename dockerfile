...
# procps provides `ps` for process debugging (optional, but useful)
RUN apt-get update \
...
CMD sh -c 'if [ ! -s /data/accounts.db ]; then cp -f /seed/accounts.db /data/accounts.db; fi && npm run start:combined'

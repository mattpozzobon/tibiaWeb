...
import { WebsocketSocketHandler } from "./websocket-server-socket-handler";
...
class WebsocketServer {
  ...
  private __removePlayer(gameSocket: GameSocket) {
    ...
    // Always save player data when they disconnect, regardless of server status
    // This ensures players don't lose progress during maintenance or shutdown
    const playerName = gameSocket.player?.getProperty(CONST.PROPERTIES.NAME);
    const gameServer = getGameServer();
    const isMaintenanceOrShutdown = gameServer.statusManager.isShutdown() || 
                                    gameServer.statusManager.isClosed() || 
                                    gameServer.statusManager.isMaintenance();
    
    if (isMaintenanceOrShutdown) {
      console.log(`Player ${playerName} disconnected during server shutdown/maintenance - saving data...`);
    }

    this.accountDatabase.updateCharacterData(id, JSON.stringify(gameSocket.player), () => {
      if (isMaintenanceOrShutdown) {
        console.log(`Player ${playerName} data saved successfully.`);
      }
    });
  }

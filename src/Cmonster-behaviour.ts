...
class MonsterBehaviour implements IMonsterBehaviour{
  ...
  private __findTarget(): void {
    const chunks = getGameServer().world.getSpectatingChunks(this.monster.getPosition());
  ...
  private __canReach(targetPosition: Position): boolean {
    ...
    if (targetPosition.besides(this.monster.getPosition())) return true;

    const path = getGameServer().world.findPath(this.monster, this.monster.getPosition(), targetPosition, Pathfinder.ADJACENT);

...
AccountDatabase.prototype.__handleOpen = function(error) {
  ...
  console.log("The sqlite database connection to %s has been opened".format(this.filepath));
...
AccountDatabase.prototype.__open = function(callback) {
  ...
  if(fs.existsSync(this.filepath)) {
    return this.db = new sqlite3.Database(this.filepath, sqlite3.OPEN_READWRITE, callback);
  }
...
AccountDatabase.prototype.__createDefaultCharacter = function(DEFAULT_CHARACTER) {
  ...
  this.createAccount(queryObject, function(error) {
    ...
    if(error !== null) {
      return console.error("Error creating default character: %s".format(error));
    }

    console.log("Default character %s has been created".format(queryObject.name));
...
AccountDatabase.prototype.createAccount = function(queryObject, callback) {
  ...
  this.getAccountCredentials(queryObject.account, function(error, result) {
    ...
    bcrypt.hash(queryObject.password, SALT_ROUNDS, function(error, hash) {
      ...
      this.db.run("INSERT INTO accounts(account, hash, name, character) VALUES(?, ?, ?, ?)", values, callback);
